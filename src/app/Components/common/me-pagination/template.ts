...
<ul class="ngx-pagination right"
    role="navigation"
    [attr.aria-label]="screenReaderPaginationLabel"
    *ngIf="!(autoHide && p.pages.length <= 1)">
    <li  class="waves-effect " [class.disabled]="p.isFirstPage()" *ngIf="directionLinks">
        <a *ngIf="1 < p.getCurrent()" (click)="p.previous()" [attr.aria-label]="previousLabel + ' ' + screenReaderPageLabel">
         <i class="fa fa-step-backward secondaryTextColor" aria-hidden="true"></i>
         <span class="show-for-sr">{{ screenReaderPageLabel }}</span>

            </a>
        <span *ngIf="p.isFirstPage()">
        <li class="waves-effect "> <i class="fa fa-step-backward secondaryTextColor" aria-hidden="true"></i></li>
        <span class="show-for-sr">{{ screenReaderPageLabel }}</span>
        </span>
    </li>
    ...
    <li class="waves-effect" [class.disabled]="p.isLastPage()" *ngIf="directionLinks">
        <a *ngIf="!p.isLastPage()" (click)="p.next()" [attr.aria-label]="nextLabel + ' ' + screenReaderPageLabel">
        <i class="fa fa-step-forward secondaryTextColor" aria-hidden="true"></i>
        </a>
