...
@Component({
  selector   : "app-me-add-balance",
  templateUrl: "./me-add-balance.component.html",
  styleUrls: ["./me-add-balance.component.css"]
})
export class MeAddBalanceComponent implements OnInit {
  ...
  constructor(
      ...
      private addBalancePersistenceService: PersistenceService,
   ...
   console.log( this.customer_number_param);
    ...
    this.balanceAmount = this.addBalance.controls["balanceAmount"];
    this.balanceRemarks = this.addBalance.controls["balanceRemarks"];
    ...
    this.balanceAmount.valueChanges.subscribe((value: string) => {
      console.log(
        "%c ------------------  Event Change balanceAmount ------------------",
        "background: green; color: white; display: block;"
      );
      console.log("BalanceAmount Value:", value);
this.checkValue=value;
      this.ShowBalanceAmount=false;
      if(value){
  ...
  ngOnInit() {
    ...
    this._userLogin = this.addBalancePersistenceService.get(
      "userLogin",
      StorageType.SESSION
    );
  ...
  onSubmit(value: string): void {
    ...
    */
    if(!this.addBalance.dirty){
      this.notification.ShowPreDefinedMessage('I', 'CMN-002');
  }
 
    else if (this.addBalance.valid && !this.ShowBalanceAmount) {
       ...
       console.log(this.formValuesJSON); 
        ...
        this.postRequestObject.records[0].customer_details.comments        = 
                              this._userProfileData.userDetails.user_name                        + " added payment " +
                              this.formValuesJSON.balanceAmount                                      + " on " + this.currentDate; 

        this.postRequestObject.records[0].payment_details.payment_amount   =       this.formValuesJSON.balanceAmount;
        this.postRequestObject.records[0].payment_details.payment_remarks  =       this.formValuesJSON.balanceRemarks;
        this.postRequestObject.records[0].payment_details.payment_type     =       'ADDBAL';
        this.postRequestObject.records[0].payment_details.payment_action   =       'ADDBAL';
       ...
       this.validateFields.validateAllFormFields(this.addBalance);
